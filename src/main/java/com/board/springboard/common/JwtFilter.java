...
/**
 * 브라우저가 서버에 요청을 보낼 때 마다 자동으로 실행
 * 쿠키에서 액세스 토큰을 꺼내서 유효하면 로그인상태로 등록
 *
 */
public class JwtFilter extends OncePerRequestFilter {
     ...
     * 이용가능                     Internal = Spring이 만든 내부 기능을
     * ====================================================================================
     * @param 요청                  브라우저가 보낸 요청 경로
     * @param 응답                  브라우저 요청에 대한 응답을 서버가 전달할 때 들고갈 정보
     * @param 다음필터                  우리 필터 작업은 끝났고, 다음 작업으로 바통터치할 때 사용
     * @throws ServletException         서버연결 에러 발생하는 경우
     * @throws IOException                  데이터 주고 받을 때 발행하는 경우
     */
    ...
    protected void doFilterInternal(HttpServletRequest 요청, HttpServletResponse 응답, FilterChain 다음필터) throws ServletException, IOException {
        ...
        if (token !=null && jwtUtil.유효토큰인지확인하는기능(token)){
            ...
            UsernamePasswordAuthenticationToken 인증정보 = //비밀번호 (이미 검증했으니 필요없음)
                    new UsernamePasswordAuthenticationToken(
                            email,
                            null,
                            List.of(new SimpleGrantedAuthority("ROLE_USER")) //권한 목록들 중에 일반 유저라는 목록이 있는가?
                    );
            SecurityContextHolder.getContext().setAuthentication(인증정보);
        }
        다음필터.doFilter(요청,응답); //바통 넘기기
        //로그인이 되었든 비로그인이 되었든 거증 할 일은 다 끝냈으니 일 할거있으면 해
        //차단은 SecurityConfig가 담당
    }
    private String 쿠키에서토큰꺼내기(HttpServletRequest 요청){
            Cookie[] 쿠키목록 = 요청.getCookies(); //요청에 담긴 모든 쿠키 가져오기
        if(쿠키목록 ==null) return null; // 쿠키 자체가 없으면 null 변환

        //쿠키 목록들을 스트림으로 변환
        return Arrays.stream(쿠키목록)
                .filter(c-> "access_token".equals(c.getName())) //access_token 이름인 쿠키만 통과
                .map(Cookie::getValue)//쿠키객체에서 값(토큰 문자열)만 꺼내기
