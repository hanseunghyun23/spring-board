        ...
        <button class="navbar-toggler" type="button"
            ...
            <ul class="navbar-nav ms-auto">
                ...
                <c:if test="${not empty sessionScope.loginUser}">
                    <li class="nav-item">
                        <a class="nav-link" href="/user/profile"><span id="유저이름">님 환영해요!</span></a>
...
<script>
    //async 비동기= 이 코드 안에 잠시 멈추고 백엔드 결과를 기다려야한다는 코드가 들어있다.
        ...
        try{
         const res = await  fetch("/user/profile-info", {method:"GET"})
            if(res.ok){
                ...
                //querySelector 이용해 .아이디이름 #클래스이름 태그이름을 모두 작성할 수 있는 기능
                //css문법에서 아이디이름 클래스이름 태그이름 사용을 그대로 똑같이 사용
          }
        }
